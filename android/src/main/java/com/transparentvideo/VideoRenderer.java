 ...
 */
 ...
 class VideoRenderer implements GLTextureView.Renderer, SurfaceTexture.OnFrameAvailableListener {
     ...
     // Fragment Shader Code for Alpha Transparency
     private static final String alphaShader = "#extension GL_OES_EGL_image_external : require\n" +
             ...
             "  float isMatch = step(length(color.rgb - targetColor), accuracy);\n" +
             "  gl_FragColor = vec4(color.rgb, 1.0 - isMatch);\n" + // Set alpha based on match
             "}\n";

     private static final String alphaPackedShader = "#extension GL_OES_EGL_image_external : require\n" +
             "precision mediump float;\n" +
             "varying vec2 vTextureCoord;\n" +
             "uniform samplerExternalOES sTexture;\n" +
             "void main() {\n" +
             "  vec4 color = texture2D(sTexture, vec2(vTextureCoord.x, vTextureCoord.y * 0.5));\n" + // Adjust for packed
                                                                                                     // texture
             "  float alpha = texture2D(sTexture, vec2(vTextureCoord.x, 0.5 + vTextureCoord.y * 0.5)).r;\n" + // Fetch
                                                                                                              // alpha
                                                                                                              // from
                                                                                                              // second
                                                                                                              // part
     ...
     /**
      * True if the video is an alpha packed video with the left half containing the
      * color data
     ...
     private String resolveShader() {
         return isCustom ? shader
                 : isPacked ? alphaPackedShader
                         : String.format(Locale.ENGLISH, alphaShader,
